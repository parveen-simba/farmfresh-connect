...
import { getProducts, getCurrentUser, getOrdersByBuyer, logout } from "@/lib/store";
import { formatPrice, getCategoryEmoji, freshnessLabel } from "@/lib/helpers";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Search, ShoppingCart, LogOut, MapPin, ClipboardList } from "lucide-react";
...
const Marketplace = () => {
  ...
  const filtered = products.filter((p) => {
    const matchesSearch = p.name.toLowerCase().includes(search.toLowerCase()) || p.farmerName.toLowerCase().includes(search.toLowerCase());
      ...
      <div className="bg-warm-gradient text-secondary-foreground px-4 pt-6 pb-6 rounded-b-3xl">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="ghost" size="icon" className="text-secondary-foreground hover:bg-secondary-foreground/10" onClick={() => { logout(); navigate("/"); }}>
        ...
        <div className="relative">
          ...
          <Input
            ...
            className="pl-10 h-12 rounded-xl bg-background text-foreground border-0"
          />
        </div>
      </div>
      ...
      {showOrders ? (
        <div className="px-4 mt-4 space-y-3">
          ...
          {orders.map((o) => (
            <div key={o.id} className="bg-card rounded-xl p-4 shadow-sm">
              ...
              <p className="text-sm text-muted-foreground mt-1">From: {o.farmerName} • {o.quantity} units • {formatPrice(o.totalPrice)}</p>
            </div>
          ))}
        </div>
      ) : (
        <>
          ...
          <div className="flex gap-2 px-4 mt-4 overflow-x-auto pb-2 no-scrollbar">
            {categories.map((c) => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${selectedCategory === c ? "bg-primary text-primary-foreground" : "bg-card text-muted-foreground"}`}
              >
          ...
          <div className="px-4 mt-4 grid grid-cols-2 gap-3">
            {filtered.map((p, i) => {
              const freshness = freshnessLabel(p.harvestDate);
              return (
                <motion.div
                  key={p.id}
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground mt-1">by {p.farmerName}</p>
                </motion.div>
              );
            })}
          </div>
